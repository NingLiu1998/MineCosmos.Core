    ...
    public static class CustomSwaggerSetup
    {
        public static void AddCustomSwaggerSetup(this IServiceCollection services)
        {
            ...
            services.AddSwaggerGen(c =>
            {
                ...
                var xmlPath = Path.Combine(basePath, "MineCosmos.Core.Gateway.xml");
        ...
        public static void UseCustomSwaggerMildd(this IApplicationBuilder app)
        {
            ...
            {
                options.SwaggerEndpoint($"/swagger/v1/swagger.json", $"MineCosmos.Core.Gateway-v1");

                apis.ForEach(m =>
                {
                    options.SwaggerEndpoint($"/swagger/apiswg/{m}/swagger.json", m);
                    options.IndexStream = () => app.GetType().GetTypeInfo().Assembly.GetManifestResourceStream("MineCosmos.Core.ApiGateway.index.html");
                });
