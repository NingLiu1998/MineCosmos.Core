            ...
            services.AddSwaggerGen(c =>
            {
                ...
                typeof(ApiVersions).GetEnumNames().ToList().ForEach(version =>
                {
                    c.SwaggerDoc(version, new OpenApiInfo
                    {
                        ...
                        Contact = new OpenApiContact { Name = ApiName, Email = "MineCosmos.Core@xxx.com", Url = new Uri("https://neters.club") },
                ...
                {
                    //这个就是刚刚配置的xml文件名
                    var xmlPath = Path.Combine(basePath, "MineCosmos.Core.xml");
                    //默认的第二个参数是false，这个是controller的注释，记得修改
                    c.IncludeXmlComments(xmlPath, true);

                    //这个就是Model层的xml文件名
                    var xmlModelPath = Path.Combine(basePath, "MineCosmos.Core.Model.xml");
                    c.IncludeXmlComments(xmlModelPath);
                }
                catch (Exception ex)
                {
                    log.Error("MineCosmos.Core.xml和MineCosmos.Core.Model.xml 丢失，请检查并拷贝。\n" + ex.Message);
                }
                    ...
                    c.AddSecurityDefinition("oauth2", new OpenApiSecurityScheme
                    {
                        ...
                        Flows = new OpenApiOAuthFlows
                        {
                            Implicit = new OpenApiOAuthFlow
                            {
                                ...
                                {
                                    "minecosmos.core.api","ApiResource id"
                                }
                            }
                            }
                        }
